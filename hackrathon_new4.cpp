...
int saturation_degree(int node, const vector<int>& color, const vector<vector<int>>& adj) {
...
vector<int> DSatTur(const vector<vector<int>>& adj) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        for (int j = 0; j < n; j++) {
            if (color[j] == -1) {
                int saturation = saturation_degree(j, color, adj);
                if (saturation > max_saturation || (saturation == max_saturation && adj[j].size() > max_degree)) {
                    max_saturation = saturation;
                    max_degree = adj[j].size();
                    chosen_node = j;
                }
            }
        }
